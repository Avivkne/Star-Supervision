...
# עיצוב מותאם לנייד + הקטנת כותרות במסכים קטנים
st.markdown("""
    <style>
    ...
    .footer-credit {
        position: fixed; left: 20px; bottom: 10px; text-align: left;
        direction: ltr; color: #888888; font-size: 14px;
        font-family: 'Segoe UI', Tahoma, sans-serif; font-weight: bold;
    }
    </style>
...
for idx, item in enumerate(st.session_state.dynamic_remarks):
    ...
    st.session_state.dynamic_remarks[idx]['text'] = st.text_area("פירוט הליקוי:", value=item['text'], key=f"text_{idx}", label_visibility="collapsed")
    st.session_state.dynamic_remarks[idx]['image'] = st.file_uploader(f"תמונה ל-4.{idx + 1}:", type=["png", "jpg", "jpeg"], key=f"image_{idx}")
    st.write("---")

if st.button("➕ הוסף הערה ותמונה"):
...
if st.button("🚀 הפק קובץ Word"):
    try:
        ...
        context = {
            ...
            'specific_remarks_list': spec_remarks, 'general_remarks_list': gen_remarks, 'cc_final_list': cc_final
        }
